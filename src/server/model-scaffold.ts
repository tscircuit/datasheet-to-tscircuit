...
4. Create a baseline model and one executable tscircuit test bench per benchmark.
   ...
   \`candidates/<revision>/model.lib\`. Use \`tsci simulate analog <file>\` for
   supported transient tests.
...
- \`results/verified/<benchmark-id>.csv\`: server-owned results extracted from
  Circuit JSON. Never create or edit this directory.
...
\`\`\`json
{
  ...
  "benchmarks": [{
    ...
    "simulation": {
      "kind": "probe_sweep",
      "points": [
        { "x": 0, "probe_name": "RESULT_0", "reducer": "tail_mean", "scale": 1 },
        { "x": 1, "probe_name": "RESULT_1", "reducer": "tail_mean", "scale": 1 }
      ]
    }
  }]
}
...
milliseconds. For a parameter sweep, use \`kind: "probe_sweep"\` and make the
benchmark circuit simulate every listed operating point with a uniquely named
\`<voltageprobe>\`. Points support \`last\`, \`tail_mean\`, \`peak_to_peak\`, or
...
validation-feedback.md exists, fix every item in it and rerun the locked suite.
