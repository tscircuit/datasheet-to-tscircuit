...
test("three-way geometry disagreement publishes the best-supported output with warnings", async () => {
...
`,
  ...
  expect(job?.warnings?.join("\n")).toContain("independent-1 versus independent-2")
  expect(job?.warnings?.join("\n")).toContain("independent-2 versus independent-3")
  expect(job?.warnings?.join("\n")).toContain("independent-3 versus independent-4")
  expect(
    await Bun.file(join(job_dir, "evidence-attempts/independent-4/component-evidence.json")).exists(),
  ).toBe(true)
  expect(job?.logs.map((log) => log.message).join("\n")).toContain(
